...
<body>
	...
	<div class="modal fade" id="myModal" tabindex="-1" role="dialog"
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				...
				<div class="modal-body">
					<form class="form-horizontal">
						<div class="form-group">
							...
							<div class="col-sm-10">
								<input type="text" name="newsId" class="form-control"
									id="ID_input" placeholder="请输入新闻ID"> <span
									class="help-block"></span>
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-sm-10">
								<input type="text" name="title" class="form-control"
									id="Name_input" placeholder="请输入新闻标题"> <span
									class="help-block"></span>
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-sm-10">
								<label class="radio-inline"> <input type="radio"
									name="newsType1" id="Kind1_input" value="1" checked="checked">
									<!-- values指定提交值 --> 工作资讯
								</label> <label class="radio-inline"> <input type="radio"
									name="newsType1" id="Kind2_input" value="2"> 学习交流
								</label> </label> <label class="radio-inline"> <input type="radio"
									name="newsType1" id="Kind3_input" value="3"> 生活资讯
								</label>
							</div>
						</div>
						<div class="form-group"></div>
						<div class="form-group">
							...
							<div class="col-sm-10">
								<input type="text" name="newsContent" class="form-control"
									id="Link_input" placeholder="请输入新闻链接"> <span
									class="help-block"></span>
							</div>
						</div>
					</form>
				...
				<div class="modal-footer">
					<button type="button" class="btn btn-primary" id="news_uptate_btn">发布</button>
					<button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
				</div>
			</div>
		</div>
	...
	<div id="wrapper">
		...
		<div id="page-wrapper">
			<div class="header">
				...
				<button type="button" class="btn btn-success" id="news_input_btn"
	...
	$("#ID_input").change(function(){
		...
		$.ajax({
			url:"${APP_PATH}/checknewsId",
			data: "newsId="+newsId,
			type:"POST",
			success:function(result){
				if(result.code==100){
					show_validate_msg("#ID_input","success","新闻ID可用");
					$("#news_uptate_btn").attr("ajax_va","success"); //给按钮添加一个属性 内容为success
				}else{
					show_validate_msg("#ID_input","error",result.extend.va_msg);
					$("#news_uptate_btn").attr("ajax_va","error");
				}
			}		 
		});
	});
	...
	$("#Name_input").change(function(){
		...
		$.ajax({
			...
			success:function(result){
				if(result.code==100){
					show_validate_msg("#Name_input","success","新闻标题可用");
					$("#news_uptate_btn").attr("ajax_va","success"); //给按钮添加一个属性 内容为success
				}else{
	...
	$("#Link_input").change(function(){
		...
		var title = this.value;
		$.ajax({
			url:"${APP_PATH}/checknewsContent",
			data: "newsContent="+newsContent,
			type:"POST",
			success:function(result){
				if(result.code==100){
					show_validate_msg("#Link_input","success","新闻链接可用");
					$("#news_uptate_btn").attr("ajax_va","success"); //给按钮添加一个属性 内容为success
				}else{
					show_validate_msg("#Link_input","error",result.extend.va_msg);
					$("#news_uptate_btn").attr("ajax_va","error");
				}
			}		 
		});
	...
	$("#news_uptate_btn").click(function(){
		$.ajax({
			url:"${APP_PATH}/new",
			type:"POST",
			...
			success:function(result){
				if(result.code == 100){
					...
					//显示失败信息
					//有哪个字段的错误信息就显示哪个字段
					if(result.extend.va_msg1=="新闻ID不能为空"){
						show_validate_msg("#ID_input","error",result.extend.va_msg1);
					}else if(result.extend.va_msg2=="新闻标题不能为空"){
						show_validate_msg("#Name_input","error",result.extend.va_msg2);
					}else if(result.extend.va_msg3=="新闻链接不能为空"){
						show_validate_msg("#Link_input","error",result.extend.va_msg3);
					}else{
						if(undefined != result.extend.errorFields.newsId){
						//显示员工名字的错误信息
						show_validate_msg("#ID_input","error",result.extend.errorFields.newsId);
						}
						if(undefined != result.extend.errorFields.title){
						//显示员工名字的错误信息
						show_validate_msg("#Name_input","error",result.extend.errorFields.title);
						}
						if(undefined != result.extend.errorFields.newsContent){
						//显示员工名字的错误信息
						show_validate_msg("#Name_input","error",result.extend.errorFields.newsContent);
						}
					}
				} 	
			}
	...
	$("#news_input_btn").click(function() {
		//清除表单数据(表单完整重置(表单的数据，表单的样式))
		reset_form("#myModal form");
		$("#myModal form")[0].reset();
		//弹出模态框
		$("#myModal").modal({ //利用js创建模态框
			backdrop : "static"
		});
	});
	...
	$("#news_uptate_input_btn").click(function(){
		$.ajax({
			url:"${APP_PATH}/new/"+$(this).attr("edit-id"),
			type:"POST",
